...
func (l *GetByPathLogic) GetByPath(in *post.GetByPathReq) (*post.Post, error) {
	...
	imageModelList, err := l.svcCtx.ImageModel.ListByPostUuid(l.ctx, postModel.Uuid)
	if errors.Is(err, model.ErrNotFound) {
	...
	authorModel, err := l.svcCtx.AuthorModel.FindOneByPostUuid(l.ctx, postModel.Uuid)
	if errors.Is(err, model.ErrNotFound) {
	...
	labelModelList, err := l.svcCtx.LabelModel.ListByPostUuid(l.ctx, postModel.Uuid)
	if errors.Is(err, model.ErrNotFound) {
	...
	locationModel, err := l.svcCtx.LocationModel.FindOneByPostUuid(l.ctx, postModel.Uuid)
	if errors.Is(err, model.ErrNotFound) {
