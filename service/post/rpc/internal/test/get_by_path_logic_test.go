...
func TestGetByPath(t *testing.T) {
	...
	logicService := logic.NewGetByPathLogic(ctx, &svc.ServiceContext{
	...
	postRepo.EXPECT().FindOneByUrl(ctx, postUrl).Return(nil, model.ErrNotFound)
	...
	postRepo.EXPECT().FindOneByUrl(ctx, postUrl).Return(nil, expectedErr)
	actual, actualErr = logicService.GetByPath(getByPathReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
	...
	actual, actualErr = logicService.GetByPath(&post.GetByPathReq{BlogId: uuid.NewString(), Path: postUrl})
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
