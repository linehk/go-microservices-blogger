...
func TestListByBlog(t *testing.T) {
	...
	logicService := logic.NewListByBlogLogic(ctx, &svc.ServiceContext{
	...
	expected := &comment.ListByBlogResp{
	...
	commentRepo.EXPECT().ListByBlogUuid(ctx, blogId).Return(nil, model.ErrNotFound)
	actual, actualErr := logicService.ListByBlog(listByBlogReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)

	// Database
	expectedErr = errcode.Wrap(errcode.Database)
	commentRepo.EXPECT().ListByBlogUuid(ctx, blogId).Return(nil, expectedErr)
	actual, actualErr = logicService.ListByBlog(listByBlogReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
	...
	commentRepo.EXPECT().ListByBlogUuid(ctx, blogId).Return(commentModelList, nil)
