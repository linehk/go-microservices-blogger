...
func TestGetBlogUserInfos(t *testing.T) {
	ctrl := gomock.NewController(t)
	ctx := context.Background()
	userBlogInfoRepo := model.NewMockBlogUserInfoModel(ctrl)
	...
	logicService := logic.NewGetBlogUserInfosLogic(ctx, &svc.ServiceContext{
		BlogUserInfoModel: userBlogInfoRepo,
		BlogModel:         blogRepo,
	...
	// BlogUserInfoNotExist
	expectedErr := errcode.Wrap(errcode.BlogUserInfoNotExist)
	userBlogInfoRepo.EXPECT().FindOneByUserUuidAndBlogUuid(ctx, userId, blogId).Return(nil, model.ErrNotFound)
	actual, actualErr := logicService.GetBlogUserInfos(getBlogUserInfosReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)

	// Database
	expectedErr = errcode.Wrap(errcode.Database)
	userBlogInfoRepo.EXPECT().FindOneByUserUuidAndBlogUuid(ctx, userId, blogId).Return(nil, expectedErr)
	actual, actualErr = logicService.GetBlogUserInfos(getBlogUserInfosReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
	...
	userBlogInfoRepo.EXPECT().FindOneByUserUuidAndBlogUuid(ctx, userId, blogId).Return(userBlogInfoModel, nil)
	actual, actualErr = logicService.GetBlogUserInfos(getBlogUserInfosReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
