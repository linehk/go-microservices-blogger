...
func TestListPostUserInfos(t *testing.T) {
	...
	logicService := logic.NewListPostUserInfosLogic(ctx, &svc.ServiceContext{
	...
	postUserInfoUuid1 := uuid.NewString()
	postUserInfoUuid2 := uuid.NewString()
	postId1 := uuid.NewString()
	postId2 := uuid.NewString()
	hasEditAccess1 := true
	hasEditAccess2 := true
	postUserInfosModelList := []*model.PostUserInfo{
		{
			Id:            1,
			Uuid:          postUserInfoUuid1,
			...
			HasEditAccess: sql.NullBool{Bool: hasEditAccess1, Valid: true},
		}, {
			Id:            2,
			Uuid:          postUserInfoUuid2,
			UserUuid:      sql.NullString{String: userId, Valid: true},
			BlogUuid:      sql.NullString{String: blogId, Valid: true},
			PostUuid:      sql.NullString{String: postId2, Valid: true},
			HasEditAccess: sql.NullBool{Bool: hasEditAccess2, Valid: true},
		},
	}
	...
	expected := &post.ListPostUserInfosResp{
		Kind:          "blogger#postUserInfosList",
		NextPageToken: "",
		Items: []*post.PostUserInfos{
			{
	...
	postUserInfoRepo.EXPECT().ListByUserUuidAndBlogUuid(ctx, userId, blogId).Return(nil, model.ErrNotFound)
	actual, actualErr := logicService.ListPostUserInfos(listPostUserInfosReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)

	// Database
	expectedErr = errcode.Wrap(errcode.Database)
	postUserInfoRepo.EXPECT().ListByUserUuidAndBlogUuid(ctx, userId, blogId).Return(nil, expectedErr)
	actual, actualErr = logicService.ListPostUserInfos(listPostUserInfosReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
	...
	postUserInfoRepo.EXPECT().ListByUserUuidAndBlogUuid(ctx, userId, blogId).Return(postUserInfosModelList, nil)
	postRepo.EXPECT().FindOneByBlogUuidAndPostUuid(ctx, blogId, postId1).Return(nil, model.ErrNotFound)
	actual, actualErr = logicService.ListPostUserInfos(listPostUserInfosReq)
	assert.Nil(t, actual)
	assert.Equal(t, expectedErr, actualErr)
	...
	postRepo.EXPECT().FindOneByBlogUuidAndPostUuid(ctx, blogId, postId1).Return(postModel1, nil)
	...
	postRepo.EXPECT().FindOneByBlogUuidAndPostUuid(ctx, blogId, postId2).Return(postModel2, nil)
